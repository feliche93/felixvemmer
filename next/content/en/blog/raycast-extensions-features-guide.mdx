...
Despite my brief encounter with Alfred, I delved into a [Reddit discussion](https://www.reddit.com/r/macapps/comments/11c9zjg/raycast_vs_alfred_opinions/) to grasp how other users perceive these two tools. Here's a synthesis of the user opinions from the discussion:

### Efficiency and Usability

- **Alfred** is often cited for its efficiency, requiring fewer keystrokes to accomplish tasks which accelerates the user experience. Particularly, its **folder navigation and file move or copy actions** are considered more ergonomic and in tune with MacOS's native interactions.
- **Raycast**, while **modern and sleek in design**, is noted to require more setup in terms of shortcuts to match Alfred's efficiency, especially in folder navigation.

### Functionality and Customization

- Alfred is recognized for its **ease of setting up actions and workflows with a no-code editor**, which is beneficial for those not versed in coding. Its long-standing presence has birthed a robust community and a **plethora of available workflows**.
- Raycast, conversely, is **appreciated for specific functions** like date comparisons, baked-in unit converter, and calculator. However, its extension/workflow creation is tied to React.js, which might pose a challenge for those unfamiliar with the framework.

### Community and Extensions

- Alfred's **longstanding community** provides a wealth of workflows and examples which can be a boon for new users.
- Raycast, although newer, has a **growing community and an easily accessible store for third-party extensions**. Users find it easier to find and add extensions in Raycast, which adds to its appeal.
...
- **Raycast is free**, which is attractive, but there's a hint of skepticism regarding data collection or future monetization, although it already has a paid tier for teams and pro.
...
The discussion suggests that the choice between Alfred and Raycast **boils down to personal preferences, use cases, and the level of customization or coding expertise a user possesses**. Both launchers have their strengths and cater to different user needs and experiences.
